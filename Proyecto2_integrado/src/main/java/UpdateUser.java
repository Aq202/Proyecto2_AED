...
public class UpdateUser extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (id == null)
				throw new BadRequestException("La propiedad 'id' es requerida.");
			if (name == null)
			...
			try {
				...
				try (User user = new User()) {
					String result = user.updateUser(id, name.trim(), sex.toUpperCase().trim(), nationality.toUpperCase().trim(), language.toUpperCase().trim(), birthYear);
					
					myResponse.put("result", result);
					myResponse.put("userId", id); //enviar userId
